...
const SubmenuPortal = ({ x, y, children, onClose }) => {
  ...
  return createPortal(
    <div
      ref={portalRef}
      className="fixed bg-gray-300 dark:bg-sidebar shadow-xl p-2 min-w-[200px] rounded-lg z-[9999]"
      style={{ top: y, left: x }}
    >
      {children}
    </div>,
...
export const Sidebar = () => {
  ...
  const [submenuPortal, setSubmenuPortal] = useState({
    show: false,
    x: 0,
    y: 0,
    content: null,
  ...
  return (
    <>
      {submenuPortal.show && (
        ...
        >
          <ul>
            {submenuPortal.content.map((subItem, subIndex) => (
